...
export class BaseChart implements OnInit, OnDestroy {
    ...
    createColor(i, base, factor = 10):number {
        return Math.abs(base - i * factor) % 255;
    }

    getColour(i):any {
        let colour = [this.createColor(i, 70), this.createColor(i, 191), this.createColor(i, 189)];
        return {
    ...
    private refresh() {
        ...
        for (let i = 0; i < this._data.length; i++) {
            ...
            let colour = i < this.colours.length ? this.colours[i] : this.defaultsColours[i] || this.getColour(i);


            let data:any = (<any>Object).assign(colour,
                this.getDataObject(this.series[i] || this.labels[i], this._data[i]));
