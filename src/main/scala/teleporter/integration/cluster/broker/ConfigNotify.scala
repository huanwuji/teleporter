  ...
  override def receive: Receive = {
    case Upsert(key) ⇒
      Keys.table(key) match {
        case "task" ⇒ notifyTask(key, ConfigChangeNotify.Action.UPSERT)
        case "group" ⇒ notifyGroup(key, ConfigChangeNotify.Action.UPDATE)
        case "partition" ⇒ notifyPartition(key, ConfigChangeNotify.Action.UPDATE)
        case "stream" ⇒ notifyStream(key, key, ConfigChangeNotify.Action.UPDATE)
        case "source" ⇒ notifySource(key, key, ConfigChangeNotify.Action.UPDATE)
        case "sink" ⇒ notifySink(key, key, ConfigChangeNotify.Action.UPDATE)
        case "address" ⇒ notifyAddress(key, key, ConfigChangeNotify.Action.UPDATE)
      }
    case Remove(key) ⇒
      Keys.table(key) match {
        case "task" ⇒ notifyTask(key, ConfigChangeNotify.Action.REMOVE)
        case "group" ⇒ notifyGroup(key, ConfigChangeNotify.Action.REMOVE)
        case "partition" ⇒ notifyPartition(key, ConfigChangeNotify.Action.REMOVE)
        case "stream" ⇒ notifyStream(key, key, ConfigChangeNotify.Action.REMOVE)
        case "source" ⇒ notifySource(key, key, ConfigChangeNotify.Action.REMOVE)
        case "sink" ⇒ notifySink(key, key, ConfigChangeNotify.Action.REMOVE)
        case "address" ⇒ notifyAddress(key, key, ConfigChangeNotify.Action.REMOVE)
      }
  }
